...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    return_list = []
    for dic in statistics:
        return_list.append((dic[info['playerid']], formula(info, dic)))
    return_list = sorted(return_list, key=lambda x: x[1], reverse=True)
    return return_list[0:numplayers]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    table = []
    stat = read_csv_as_list_dict(info['masterfile'], info['separator'], info['quote'])
    for ids in top_ids_and_stats:
        for dic in stat:
            if dic[info['playerid']] == ids[0]:
                table.append('{0:.3f} --- {1} {2}'.format(ids[1], dic[info['firstname']], dic[info['lastname']]))
    return table
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    se_year = filter_by_year(stat, year, info['yearid'])
    
    top_player = top_player_ids(info, se_year, formula, numplayers)
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for dit in statistics:
        dic[dit[playerid]] = {}
        for field in fields:
            dic[dit[playerid]][field] = 0
            dic[dit[playerid]][playerid] = dit[playerid]
    
    for key, val in dic.items():
        for dit in statistics:
            if key == dit[playerid]:
                for stat in fields:
                    dic[key][stat] += int(dit[stat])
    return dic
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    for player in player_name:
        play_stat.append((player, formula(info, indiv_player_stat[player])))
                         
    play_stat = sorted(play_stat, key = lambda x: x[1], reverse = True)
    
    return lookup_player_names(info, play_stat[0:numplayers])
