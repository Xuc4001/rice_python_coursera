...
def build_plot_dict(gdpinfo, country_list):
    ...
    """
    table = {}
    gdp_file = gdpinfo["gdpfile"]
    gdp_dat = read_csv_as_nested_dict(gdp_file, gdpinfo["country_name"], gdpinfo["separator"], gdpinfo["quote"])
    for country in country_list:
        if country in gdp_dat.keys():
            table[country] = build_plot_values(gdpinfo, gdp_dat[country])
        else:
            table[country] = []
        table[country].sort(key=lambda x: x[0])
    return table
